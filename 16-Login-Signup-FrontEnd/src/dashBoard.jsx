          ...
          {user.map((p) => (
            <tr key={p._id}>
              <td style={{ border: "1px solid black", padding: "8px" }}>
                {editUserId == p._id ?<input value={editUser.name} onChange={(e) => setEditUser({ ...editUser, name: e.target.value }) }/>:p.name}
              </td>
              <td style={{ border: "1px solid black", padding: "8px" }}>
                {editUserId == p._id ?<input value={editUser.email} onChange={(e) => setEditUser({ ...editUser, email: e.target.value }) }/>:p.email}
              </td>
              <td style={{ border: "1px solid black", padding: "8px" }}>
                {editUserId == p._id ?<button style={{ marginRight: "5px" }} onClick={() => handleUpdate(p._id)}>Save</button>:
                  <button style={{ marginRight: "5px" }} onClick={() => handleEdit(p)}>Edit</button>
                }
